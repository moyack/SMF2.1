...
function template_edit_file()
{
   ...
   // moyack. Some acce preparations...
   if ($context['fileinfo']['extension'] == 'js')
       $context['fileinfo']['extension'] = 'javascript';
   $ace = array(
   ...
   <div class="errorbox">
    ...
    if ($context['fileinfo']['extension'] == 'css') {
   ...
   echo '
      <form action="', $scripturl, '?action=admin;area=theme;th=', $context['theme_id'], ';sa=edit" method="post" accept-charset="', $context['character_set'], '">
         ...
         <div class="windowbg">
             <div id="debug" class="errorbox" style="visibility: hidden;"> </div>
             <div id="codeEditor" style="border: 1px solid #00000040; font-size: ', $ace['font-size'] ,'; height: ', $ace['height'] , '; margin: ', $ace['margin'] ,'">', $context['entire_file'], '
             </div>
            <textarea name="entire_file" id="entire_file" style="display: none" class="edit_file">', $context['entire_file'], '</textarea><br>
            <script>
                    var editor = ace.edit("codeEditor");
                    var annot = null;
                    editor.session.setMode("ace/mode/', $context['fileinfo']['extension'] ,'");
                    editor.setReadOnly(', $context['allow_save'] ? 'false' : 'true' ,');
                    editor.session.on("change", function(e) {
                        $("#entire_file").text(editor.getValue());
                        ...
                        if (annot.length > 0) {
                            for (var key in annot){
                                if (annot.hasOwnProperty(key))
                                    $("#debug").html("<div><strong>" + annot[key].type + "</strong> in line " + annot[key].row + ": <i>" + annot[key].text + "</i></div>");
            ...
            <input type="submit" id="save" name="save" value="', $txt['theme_edit_save'], '"', $context['allow_save'] ? '' : ' disabled', ' class="button">
            ', $context['fileinfo']['extension'] == 'css' ? '
